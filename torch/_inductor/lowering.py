...
def switch(index, branches, operands) -> list[Union[ir.TensorBox, ir.ShapeAsConstantBuffer]]:
    if any(isinstance(x, IRNode) and is_triton(x) for x in [index, *operands]):
        msg = "control flow operator: torch.switch."
        if stack_trace := V.graph.current_node.meta.get("stack_trace", None):
            msg = f"{msg} Found from : \n {stack_trace}"
        V.graph.disable_cudagraphs_reason = msg
    branch_list = list(branches) if not isinstance(branches, (list, tuple)) else branches
    result = ir.Switch.create(index, branch_list, list(operands))
